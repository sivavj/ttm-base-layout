...
export default function All() {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rowss.map((n) => n.name);
  ...
  const handleConfirmDelete = (id: any) => {
    // // setRows((rowss) => rowss.filter((id) => id != removeid))
    if (rowss.filter((data) => data !== id)) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - rowss.length) : 0;
